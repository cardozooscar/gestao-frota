...
export default function GestaoEstoquePage() {
  ...
  async function fetchProducao() {
    ...
    const { data, error } = await supabase
      ...
      .select('*, profiles:tecnico_id(nome, meta_diaria)') 
  ...
  /* ==========================================
     GRÁFICOS DA SEMANA E TOP MODELOS
     ========================================== */
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar modelo..." value={filtroModelo} onChange={(e) => setFiltroModelo(e.target.value)} className="bg-white/5 border border-white/10 rounded-2xl pl-10 pr-4 py-3 text-sm focus:border-[#2f6eea] outline-none w-full sm:w-48 transition-all" />
            </div>
            <div className="relative">
              <Barcode className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
              <input type="text" placeholder="Buscar S/N..." value={buscaSN} onChange={(e) => setBuscaSN(e.target.value)} className="bg-white/5 border border-white/10 rounded-2xl pl-10 pr-4 py-3 text-sm focus:border-[#2f6eea] outline-none w-full sm:w-56 transition-all uppercase" />
            </div>
          </div>
        </div>
