...
export default function TecnicoDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // 3. Buscar Veículo Vinculado (que não tenha data de término)
        ...
        // 4. Buscar Histórico das Últimas 3 Inspeções
        ...
        if (inspections && inspections.length > 0) {
          // Busca as placas dos veículos das inspeções no histórico
          const formattedInspections = await Promise.all(inspections.map(async (ins) => {
            const { data: v } = await supabase
              .from('vehicles')
              .select('placa')
              .eq('id', ins.vehicle_id)
              .single()
  ...
  return (
    <main className="min-h-screen bg-[#02052b] text-white p-4 pb-10 font-sans">
      <div className="mx-auto max-w-md space-y-6">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[28px] p-6 shadow-xl shadow-blue-900/30">
          <div className="relative z-10">
            ...
            {veiculoAtivo ? (
              ...
              <div>
                <p className="text-white font-bold text-lg">Nenhum veículo vinculado</p>
                <p className="text-blue-200 text-xs">Fale com o supervisor da frota.</p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[28px] overflow-hidden backdrop-blur-md">
            {ultimasInspecoes.length > 0 ? (
              ultimasInspecoes.map((ins, index) => (
                <div 
                  key={ins.id} 
